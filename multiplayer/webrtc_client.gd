...
func start(url, _player_name, _lobby = ""):
	stop()
	sealed = false
	lobby = _lobby
	connect_to_url(url, _player_name)
...
func _create_peer(id):
	...
	peer.initialize({
		"iceServers": [ { "urls": ["stun:stun.l.google.com:19302"] } ] # TODO: replace with bespoke ice server
	})
	...
	if id < rtc_mp.get_unique_id(): # this prevents the lobby creator from creating offers
		peer.create_offer()
	return peer
